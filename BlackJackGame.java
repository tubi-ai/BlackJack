...
public class BlackJackGame {
    ...
    public static BlackJack newGame = new BlackJack(gameFrame); //we initialize a 'Game' in order to control, start, and calculate the blackjack game.
    ...
    public static void openMenu() { //this method literally opens the menu.
        ...
        Option beginningComponent = new Option(); //we initialize an OptionsComponent which is the JComponent that contains the properties (buttons, titles, etc.) of the menu.
            ...
            while(true) {
                if (isFirstTime||newGame.roundOver) {//if this is the first time the game is started or the round is over (which was thanks to the checkHand method in Game),
                    ...
                    if (newGame.dealerWon){//if dealer won the game,
                        ...
                        currentBalance-= Component.currentBet; //we also subtract the bet from the current balance.
                    }
                    else {//if dealer didn't win, then the user won.
                        playerScore++; //we add one to the score of player.
                        currentBalance+= Component.currentBet*2; //we add two times the bet to the current balance.
                    }
                    gameFrame.getContentPane().removeAll(); //we remove everything from the frame.
                    newGame = new BlackJack(gameFrame); //we initialize a new game on the same frame.
