...
public class Option extends JComponent implements ActionListener{
    ...
    public Option() { //in the default constructor of this class,
    ...
    public void actionPerformed(ActionEvent e) {//we will control the button actions here.
        ...
        if(selectedButton == btnExit) { //if the selected button is the exit button (btnExit), we will exit the game.
            ...
            BlackJackGame.currentState = BlackJackGame.STATE.GAME; //we equalize the current state to STATE.GAME where STATE was the enum declared in the Tester class. Because we will no longer be in the menu, we will jump to the game.
            BlackJackGame.menuFrame.dispose(); //we first get rid of the menu frame and the components it has.
            BlackJackGame.gameRefreshThread.start(); //then, simultaneously, we start our two threads and these run at the same time.
            BlackJackGame.gameCheckThread.start();
